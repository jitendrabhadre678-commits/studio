          ...
          <div className="flex flex-col gap-4 w-full lg:w-auto">
            ...
            <div className="flex flex-wrap gap-2">
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeCategory === 'All' ? 'bg-primary text-white' : 'bg-white/5 text-white/40 hover:bg-white/10 hover:text-white border border-white/5'}`}
              >
                All
              </button>
              {categories.map(cat => (
                <button 
                  ...
                  className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-primary text-white' : 'bg-white/5 text-white/40 hover:bg-white/10 hover:text-white border border-white/5'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <AnimatePresence mode="popLayout">
            {filteredCards.map((card, idx) => {
              ...
              return (
                ...
                >
                  <Card className="glass-card border-white/5 hover:border-primary/50 transition-all duration-500 rounded-2xl h-full flex flex-col group/card shadow-2xl overflow-hidden">
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <div className="flex flex-col items-end">
                             ...
                             <span className="text-[8px] text-muted-foreground uppercase tracking-widest">Unlocked Today</span>
                          </div>
                        </div>
